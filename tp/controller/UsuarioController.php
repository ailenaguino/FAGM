...
class UsuarioController
{
    ...
    public function index()
    {
        if (isset($_SESSION['name'])) {
            ...
            $data['ejemplar'] = $this->modelEjemplar->obtenerEjemplaresConSusCategorias();
            $data['edicion'] = $this->modelEdicion->obtenerEdicionesConSuEjemplar();
            echo $this->renderer->render("view/inicioSinLogear.php", $data);
        }
    }

    public function pedirQueInicieSesion()
    {
        echo $this->renderer->render("view/loginUsuario.php");
    }
        ...
        if ($this->validarPermisosDeUsuario()) {
            ...
            if ($result) {
                ...
                foreach ($result as $v) {
                    ...
                    $this->pdf->ln();
                    $this->pdf->Cell(40, 10, "Precio: " . $v["precio"]);
                    $this->pdf->ln();
                    $this->pdf->Cell(40, 10, "Fecha de vencimiento: " . $v["vencimiento"],0,1);
                    $this->pdf->ln();
                }
                $this->pdf->SetY(250);
    ...
    public function validarLecturaPremium()
    {
        ...
        if (isset($_SESSION['name'])) {
            ...
            if ($result) {
                ...
                $result["noticia"] = $this->modelNoticia->obtenerNoticia($id_noticia);

                foreach ($result["noticia"] as $n) {
                    $id_edicion = $n["id_edicion"];
                }
                $result["edicion"] = $this->modelEdicion->obtenerEdicionPorId($id_edicion);
                $result["fotos"] = $this->modelFoto->obtenerFoto($id_noticia);

                foreach ($result["edicion"] as $n) {
                    $ejemplar = $n["id_ejemplar"];
                }
                $result["ejemplar"] = $this->modelEjemplar->obtenerEjemplarPorId($ejemplar);
                echo $this->renderer->render("view/noticiaNoComprada.php", $result);
            }
